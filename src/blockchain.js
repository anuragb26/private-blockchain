    ...
    return new Promise(async (resolve, reject) => {
      try {
        for (let block of self.chain) {
          const { star, address: ownerAddress } = await block.getBData()
        ...
        for (let i = 0; i < self.chain.length; i++) {
          ...
          if (block.height !== 0) {
            ...
            if (block.previousBlockHash !== previousBlock.hash) {
              errorLog.push(
                `Previous Block hash not equal to current hash for block with height ${block.height}`,
              )
            }
          }
        }
        const blockValidResults = await Promise.all(promises)
        for (let i = 0; i < blockValidResults.length; i++) {
          if (!blockValidResults[i]) {
            errorLog.push(`Invalid block with height ${self.chain[i].height}`)
          }
        }
        resolve(errorLog)
      } catch (error) {
